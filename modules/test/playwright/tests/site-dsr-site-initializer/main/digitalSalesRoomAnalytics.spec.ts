...
test(
	...
	async ({dsrAnalyticsPage, page}) => {
		...
		await expect(dsrAnalyticsPage.roomStatistics).toBeVisible();
		await expect(dsrAnalyticsPage.engagementTimelineFrame).not.toBeVisible();
	}
);
...
test(
	...
	async ({
		...
		await expect(dsrAnalyticsPage.roomFilterTrigger).toHaveText(/All Rooms/);

		await dsrAnalyticsPage.openRoomFilter();

		await expect(dsrAnalyticsPage.roomFilterOption('All Rooms')).toBeVisible();
...
test(
	...
	async ({
		...
		await expect(dsrAnalyticsPage.userFilterTrigger).toHaveText(/All Users/);

		await dsrAnalyticsPage.openUserFilter();

		await expect(dsrAnalyticsPage.userFilterOption('All Users')).toBeVisible();
