...
public class UserSession implements Serializable {
	...
	public UserSessionEvent[] getUserSessionEvents() {
		if (_userSessionEventsSupplier != null) {
			userSessionEvents = _userSessionEventsSupplier.get();

			_userSessionEventsSupplier = null;
		}
	...
	public void setUserSessionEvents(
		...
		_userSessionEventsSupplier = () -> {
			try {
				return userSessionEventsUnsafeSupplier.get();
			}
	...
	@JsonIgnore
	private Supplier<UserSessionEvent[]> _userSessionEventsSupplier;
	...
	public String toString() {
		...
		UserSessionEvent[] userSessionEvents = getUserSessionEvents();

		if (userSessionEvents != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0; i < userSessionEvents.length; i++) {
				sb.append(String.valueOf(userSessionEvents[i]));

				if ((i + 1) < userSessionEvents.length) {
					sb.append(", ");
				}
			}
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		...
		defaultValue = "com.liferay.site.dsr.analytics.rest.dto.v1_0.UserSession",
		name = "x-class-name"
	)
...
// LIFERAY-REST-BUILDER-HASH:-1166055634