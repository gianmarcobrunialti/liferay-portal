...
interface IAnalyticsFrameProps {
    children: any;
    helpMessage?: string;
    icon?: string;
    title?: string;
...
const AnalyticsFrame = React.forwardRef(({
    ...
    title,
    url,
}: IAnalyticsFrameProps, ref: React.ForwardedRef<HTMLElement>) => {
    return (
        <section className="analytics-frame border rounded-lg" ref={ref}>
            {(helpMessage || icon || title || url) ? (
                <header className="align-items-center border-bottom d-flex justify-content-between p-3">
                    <span className="align-items-center d-flex">
                        ...
                        <span className="font-weight-semi-bold px-2">{title}</span>
