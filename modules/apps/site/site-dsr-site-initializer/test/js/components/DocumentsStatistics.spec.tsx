...
import DocumentsStatistics from '../../../src/main/resources/META-INF/resources/js/main_view/analytics/components/DocumentsStatistics';
...
describe('DocumentsStatistics', () => {
	...
	it('renders the component with provided data', () => {
		const {baseElement} = render(
			<DocumentsStatistics items={mockData} namespace="" />
		);
	...
	it('renders the correct average time', () => {
		render(<DocumentsStatistics items={mockData} namespace="" />);

		expect(screen.getByText('1h 33 min')).toBeInTheDocument();
	});
	...
	it('handles duplicate users in user involved count', () => {
		...
		render(<DocumentsStatistics items={duplicateUserData} namespace="" />);
