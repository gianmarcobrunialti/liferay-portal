...
} from "../../info_panel/services/ObjectEntryService";
import {OBJECT_ENTRY_FOLDER_CLASS_NAME} from "../../../common/utils/constants";
import {
    IAssetTaxonomyVocabulary, ISearchAssetObjectEntry,
    ITaxonomyCategoryBrief
} from "../../../common/types/AssetType";
import {sub} from "frontend-js-web";

export function getModalDescription(
    selectedData: IBulkActionFDSData, type: string = 'TaxonomyCategoryBulkAction') {
...
export function getScopeAttributes({items = []}: IBulkActionFDSData, filterFolders = false, scopeOnly = false): Pick<EntryCategorizationDTO, 'scopeId' | 'systemProperties'> {
    ...
    if (!scopeOnly) {
        ...
        scopeAttributes.systemProperties = {
            objectDefinitionBrief: {
                classNameId: classNameIds.size === 1
                    ? [...classNameIds][0]
                    : -1,
            },
        } as any;
    ...
    return scopeAttributes as Pick<EntryCategorizationDTO, 'scopeId' | 'systemProperties'>;
}

export function toTaxonomyCategoryDTO(
    vocabularies: IAssetTaxonomyVocabulary[] = []
): Pick<EntryCategorizationDTO, 'taxonomyCategoryBriefs' | 'taxonomyCategoryIds'> {
    return vocabularies.reduce((dto, vocabulary) => {
        const {name, taxonomyCategories, taxonomyVocabularyId} = vocabulary;

        if (taxonomyCategories.length) {
            taxonomyCategories.forEach(({taxonomyCategoryId, taxonomyCategoryName}) => {
        ...
        taxonomyCategoryIds: [],
    } as Pick<EntryCategorizationDTO, 'taxonomyCategoryBriefs' | 'taxonomyCategoryIds'>);
}