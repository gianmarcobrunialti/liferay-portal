...
export function getBaseAssetInformation({
    ...
    else if (href.includes('basic-documents') || href.includes('external-videos')) {
        baseAssetInfo.icon = 'document-image';
        baseAssetInfo.type = ASSET_TYPE.FILES;
    }
    else if (href.includes('basic-web-contents') || href.includes('blogs') || href.includes('knowledge-bases')) {
