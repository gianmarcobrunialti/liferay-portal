...
function AccountSelectionModal({
	...
	const createAccount = useCallback(() => {
		...
		accountJSON.organizationIds = organizations.map(({value}) => value);
		
		DeliveryCatalogResource.createAccountByChannelId(
			commerceChannelId, accountJSON)
			.then((newAccount) => changeAccount(newAccount, !!checkoutURL))
            .catch(showErrorNotification);
	}, [accountFields, changeAccount]);
    ...
    }, [accountFields, isCreate, selectedAccountId]);
						...
						<ClayModal.Body>
							<div style={{minHeight: '14.5rem'}}>
								...
								{isCreate ? (
									...
									<ClayForm.Group>
										<label
											htmlFor="available-accounts-list">
											{Liferay.Language.get('available-accounts')}
								...
								<ClayButton.Group spaced>
									...
									<ClayButton
										...
										onClick={() =>
											isCreate
												? createAccount()
												: changeAccount({id: selectedAccountId}, true)}
										type="button"
									>
