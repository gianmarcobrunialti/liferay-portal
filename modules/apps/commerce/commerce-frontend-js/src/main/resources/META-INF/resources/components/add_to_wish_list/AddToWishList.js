...
function AddToWishList({
	...
	/**
	 * @deprecated as of 7.4 GA2
	 */
	const toggleInWishList = () => LegacyWishListResource
		.toggleInWishList(productInfo)
		.then(({success = false}) => {
			if (success) {
				setIsAdded(!isAdded);
			}
	...
	return (
		<ClayButton
			className={`btn-outline-borderless btn-${
				large ? 'lg' : 'sm'
			}`}
		...
		>
			{!iconOnly && (
				<span className={'text-truncate-inline'}>
					<span className={'text-truncate font-weight-normal'}>
			...
			<span className={'wish-list-icon'}>
				<ClayIcon spritemap={spritemap} symbol={`heart${
					isAdded
						? '-full'
						: ''
					}`}
