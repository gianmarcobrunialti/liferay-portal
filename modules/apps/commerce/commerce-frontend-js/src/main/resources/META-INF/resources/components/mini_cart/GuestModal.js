...
function GuestModal({
	isVisible,
	setIsVisible: removeModal,
	signInURL,
}) {
	const {observer, onOpenChange: setIsVisible} = useModal({
		defaultOpen: true,
		onClose: () => removeModal(false),
	});
	...
	}, [signInURL, setIsVisible, setViewsMap]);
		...
		>
			{activeView ? (
				<>
					...
					{(viewsMap[activeView].url || viewsMap[activeView].content) && (
						<>
							{alert.message && (
								<ClayAlert
