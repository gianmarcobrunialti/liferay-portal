...
function adjustModalSize({
	...
	const modalBodyIframe = window.top.document.querySelector('.modal-body-iframe');
...
function closeModal(isSuccessful, redirectURL = '') {
	...
	// window.parent.Liferay.fire('closeModal', eventDetail);
}
...
function onFormSubmit(iframeForm, event) {
	...
	if (!formWrapper || !formWrapper.formValidator || !formWrapper.formValidator.validate) {
...
export default function modalContentHandler({
	...
	const iframeWrapper = window.document.querySelector('.modal-iframe-wrapper');
	...
	if (useNativeSubmit) {
		iframeForm.addEventListener(
			'submit', onFormSubmit.bind(this, iframeForm));
	}
