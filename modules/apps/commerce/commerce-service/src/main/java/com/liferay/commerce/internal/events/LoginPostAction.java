...
public class LoginPostAction extends Action {
	...
	private AccountEntry _createAccountEntry(
			long commerceChannelGroupId, String name, String type, User user)
		throws PortalException {
		...
		if (count > 0) {
			_commerceChannelAccountEntryRelLocalService.
				addCommerceChannelAccountEntryRel(
					userId, accountEntry.getAccountEntryId(), null, 0,
					commerceChannelGroupId, false, 0,
					CommerceChannelAccountEntryRelConstants.TYPE_ELIGIBILITY);
		}
	...
	private void _updateGuestCommerceOrder(
		...
		if (userAccountEntries.isEmpty()) {
			userAccountEntries.add(
				_createAccountEntry(
					commerceChannelGroupId, user.getFullName(),
