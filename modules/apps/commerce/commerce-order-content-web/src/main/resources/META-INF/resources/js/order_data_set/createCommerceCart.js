...
const openOrderTypeSelectionModal = (orderTypes) => new Promise(
	(proceed, stop) => {
		openModal({
			bodyHTML: `
				<div class="form-group" id="orderTypeSelection">
					...
					<select class="form-control" id="orderTypeSelect">
						${orderTypes.map(
							({name_i18n, orderTypeId}) => `<option value="${orderTypeId}">${name_i18n}</option>`
						).join('')}
					</select>
				</div>
			`,
					...
					onClick: ({processClose}) => {
						...
						if (orderTypeSelect) {
							orderTypeId = parseInt(
								orderTypeSelect.selectedOptions[0], 10);
						}
...
const createCommerceCart = async ({
	...
	const onBeforeCreate = orderTypes.length > 1
	...
	onBeforeCreate(orderTypes)
		.then((orderTypeId) => {
			...
			return DeliveryCartAPI.createCartByChannelId(
				commerceChannelId, commerceCart);
		})
		.then(({id: cartId = null}) => {
			if (cartId) {
...
export default function ({
	...
	const handler = () => createCommerceCart(
		{...additionalProps, orderTypes});
