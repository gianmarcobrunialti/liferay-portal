...
public class OrdersDataSetFragmentRenderer implements FragmentRenderer {
	...
	public void render(
		...
		try {
			...
			httpServletRequest.setAttribute(
				...
				_getCommerceOrderTypes(commerceChannel.getCommerceChannelId(),
					httpServletRequest));
			httpServletRequest.setAttribute(
			...
			httpServletRequest.setAttribute(
				...
				_getFDSAdditionalProps(commerceChannel, fdsName,
					httpServletRequest));
			httpServletRequest.setAttribute(
	...
	private List<FDSActionDropdownItem> _getBulkActionDropdownItems(String fdsName, HttpServletRequest httpServletRequest) {
	...
	private Map<String, Object> _getFDSAdditionalProps(
		...
		if (fdsName.equals(CommerceOrderFragmentFDSNames.PENDING_ORDERS)) {
			...
			return HashMapBuilder.<String, Object>put(
				...
				"checkoutActionURL", PortletURLBuilder.create(
				...
				"orderDetailURL", _getCommerceOrderFriendlyURL(httpServletRequest)
			).build();
		}
	...
	private String _getAPIURL(
		long commerceChannelId, String fdsName) {

		if (fdsName.equals(CommerceOrderFragmentFDSNames.PENDING_ORDERS)) {
			return StringBundler.concat(
				"/o/headless-commerce-delivery-cart/v1.0/channels/",
				String.valueOf(commerceChannelId), "/carts");
		}
		else if (fdsName.equals(CommerceOrderFragmentFDSNames.PLACED_ORDERS)) {
			return StringBundler.concat(
				"/o/headless-commerce-delivery-order/v1.0/channels/",
				String.valueOf(commerceChannelId), "/placed-orders");
		}
	...
	private List<FDSActionDropdownItem> _getFDSActionDropdownItems(
		...
		if (fdsName.equals(CommerceOrderFragmentFDSNames.PENDING_ORDERS)) {
			return Arrays.asList(
				...
				new FDSActionDropdownItem(
					"/o/headless-commerce-delivery-cart/v1.0/carts/{id}",
					"trash", "delete", _language.get(httpServletRequest, "delete"),
					"delete", null, "async"),
				new FDSActionDropdownItem(
					StringPool.BLANK, "pencil", "rename",
					_language.get(httpServletRequest, "rename"), null,
					null, "link"),
				new FDSActionDropdownItem(
					null, "shopping-cart", "place-order",
					_language.get(httpServletRequest, "place-order"),
					null, null, null)
			);
		}
		else if (fdsName.equals(CommerceOrderFragmentFDSNames.PLACED_ORDERS)) {
			List<FDSActionDropdownItem> fdsActionDropdownItems = Arrays.asList(
				...
				new FDSActionDropdownItem(
					...
					_language.get(httpServletRequest, "reorder"), null,
					null, "link")
			);
			...
			if (FeatureFlagManagerUtil.isEnabled("LPD-10562")) {
				fdsActionDropdownItems.add(
					new FDSActionDropdownItem(
						null, "return", _language.get(httpServletRequest, "make-a-return"),
						null, null, null, null)
				);
			}
	...
	private JSONArray _getCommerceOrderTypes(
